...
const ChatBubble = ({ sender, message, index, isSaved, saveMessage }: ChatBubbleProps) => {

    const isUser = sender === SenderType.User;

    return (
        <motion.div layout variants={bubble} transition={{ duration: 0.25 }} className={cn('flex gap-4 items-start mb-3', isUser ? 'flex-row-reverse' : '')}>
